...
def load_labels(records: list[dict[str, str]]) -> list[str]:
    record_labels = {row["main_emotion"] for row in records}
    if LABEL_MAP_PATH.exists():
        label_map = json.loads(LABEL_MAP_PATH.read_text(encoding="utf-8"))
        ordered_labels = [
            label
            for label in label_map.get("labels", [])
            if label in record_labels
        ]
        extra_labels = sorted(record_labels - set(ordered_labels))
        return ordered_labels + extra_labels
    return sorted(record_labels)
...
def train(records: list[dict[str, str]]) -> dict:
    ...
    if USE_UNIFORM_LABEL_PRIOR:
        priors = {label: math.log(1 / len(labels)) for label in labels}
    else:
        priors = {
    ...
    return {
        ...
        "use_uniform_label_prior": USE_UNIFORM_LABEL_PRIOR,
    }
