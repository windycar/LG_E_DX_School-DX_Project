...
- `scripts/04_relabel_project_emotions.py`
  - 프로젝트 감정 대분류를 8개 라벨로 재구성합니다.
- `scripts/05_add_transition_training_data.py`
  - `근데`, `하지만`, `그래도`처럼 감정이 뒤에서 바뀌는 다이어리 문장을 추가 학습 데이터로 보강합니다.
- `scripts/07_add_external_diary_emotion_data.py`
  - Hugging Face 공개 한국어 다이어리 감정 데이터셋을 내려받아 현재 8개 감정 라벨로 매핑하고 추가 학습합니다.
...
AI Hub 감성대화 말뭉치의 기존 라벨은 프로젝트 라벨 체계에 맞게 재매핑했고, 부족한 라벨은 합성 다이어리 문장으로 보강했습니다.
문장 안에서 감정이 전환되는 경우에는 뒤쪽 감정 흐름을 더 크게 반영하도록 추가 학습 데이터와 예측 로직을 보강했습니다.
데이터 불균형 때문에 특정 감정으로 쏠리지 않도록 균등 라벨 사전확률을 사용합니다.
외부 공개 다이어리 감정 데이터셋도 추가해 일반적인 일기 문체와 임산부 다이어리 문체를 함께 학습합니다.
...
```cmd
build_dataset.cmd
python scripts\04_relabel_project_emotions.py
python scripts\05_add_transition_training_data.py
python scripts\07_add_external_diary_emotion_data.py
train_model.cmd
```
