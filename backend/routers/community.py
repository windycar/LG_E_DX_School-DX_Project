...
def get_community_posts(db: Session = Depends(database.get_db)):
    try:
        results = db.query(
            models.CommunityPost, models.User.name, models.User.role, models.User.pregnancy_start_date, func.count(models.CommunityComment.comment_id).label("comment_count")
        ).outerjoin(models.User, models.CommunityPost.user_id == models.User.id).outerjoin(models.CommunityComment, models.CommunityPost.post_id == models.CommunityComment.post_id).group_by(models.CommunityPost.post_id, models.User.id).order_by(models.CommunityPost.created_at.desc()).all()
        
        posts = [{"post_id": p.post_id, "user_id": p.user_id, "pregnancy_period": p.pregnancy_period, "title": p.title, "content": p.content, "created_at": p.created_at, "author": uname or "익명", "role": urole, "comment_count": count} for p, uname, urole, preg_date, count in results]
...
def get_my_posts(user_id: int, db: Session = Depends(database.get_db)):
    try:
        results = db.query(models.CommunityPost, models.User.name.label("author_name"), models.User.role.label("author_role"), func.count(models.CommunityComment.comment_id).label("comment_count")).outerjoin(models.User, models.CommunityPost.user_id == models.User.id).outerjoin(models.CommunityComment, models.CommunityPost.post_id == models.CommunityComment.post_id).filter(models.CommunityPost.user_id == user_id).group_by(models.CommunityPost.post_id, models.User.id).order_by(models.CommunityPost.created_at.desc()).all()
        return {"status": "Success", "posts": [{"post_id": p.post_id, "user_id": p.user_id, "pregnancy_period": p.pregnancy_period, "title": p.title, "content": p.content, "created_at": p.created_at, "author": aname or "익명", "role": arole, "comment_count": count} for p, aname, arole, count in results]}
    except Exception as e: return {"status": "Error", "message": str(e)}
